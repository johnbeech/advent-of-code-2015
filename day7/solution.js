...
function applyWiringInstruction (instruction, state) {
  var result = (instruction.operation) ? function () {
    ...
    state.signals[instruction.target] = wiringValue
    return state.signals[instruction.target]
  } : function () {
    ...
    state.signals[instruction.target] = inputValue
    return state.signals[instruction.target]
  }
...
function writeReport (state, suffix) {
  var outputFile = path.join(__dirname, `/day7_${suffix}.json`)
